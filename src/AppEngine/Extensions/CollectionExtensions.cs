...
public static class CollectionExtensions
{
    ...
    {
        public IEnumerable<TSource> DistinctBy<TKey>(Func<TSource, TKey> keySelector, IEqualityComparer<TKey>? comparer = null)
            => source.GroupBy(keySelector, comparer).Select(x => x.First());

        public IEnumerable<TSource[]> Chunk(int chunkSize)
        {
            if (chunkSize < 1)
            {
        ...
        public IEnumerable<TSource> ForEach(Action<TSource> action)
        {
        ...
        public async Task<IEnumerable<TSource>> ForEachAsync(Func<TSource, Task> action, CancellationToken cancellationToken = default)
        {
            foreach (var item in source)
            {
        ...
        public async Task<IEnumerable<TResult>> SelectAsync<TResult>(Func<TSource, Task<TResult>> selector, CancellationToken cancellationToken = default)
        {
        ...
        public IEnumerable<TSource> WhereIf(bool condition, Func<TSource, bool> predicate)
